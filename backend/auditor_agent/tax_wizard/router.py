...
async def upload_form16(file: UploadFile = File(...)):
    ...
    """
    # Validate file type intelligently for Vision AI
    ext = file.filename.lower().split('.')[-1]
    if ext not in ["pdf", "png", "jpg", "jpeg"]:
        raise HTTPException(
            status_code=400,
            detail="Unsupported format. Please upload your Form 16 as a PDF, PNG, or JPG."
        )

    try:
        content = await file.read()
        logger.info("Processing uploaded PDF byte stream: %s (%d bytes)", file.filename, len(content))

        # Step 1: Extract text directly from RAM bytes natively using intelligent extension parsing
        raw_text = extract_form16_text(content, ext)
