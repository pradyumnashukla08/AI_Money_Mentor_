...
function LoginContent() {
  ...
  const [errorText, setErrorText] = useState<string | null>(searchParams.get("error"));
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    if (res?.error) {
      setErrorText(res.error === "CredentialsSignin" ? "Invalid email or password" : res.error);
      setIsLoading(false);
    } else if (res?.ok) {
      router.push("/");
    }
    ...
    >
      ...
      >
        <div style={{ textAlign: "center", marginBottom: "var(--sp-8)" }}>
          ...
          >
            Sign in to access your financial advisor and save your chat history.
          </p>
        </div>
        ...
        {errorText && (
          ...
          >
            {errorText === "OAuthSignin" 
              ? "Google Authentication failed. Please make sure you have added your Google API credentials to .env.local." 
              : errorText}
          </div>
        )}

        <form onSubmit={handleCredentialsLogin} style={{ display: "flex", flexDirection: "column", gap: "var(--sp-4)" }}>
          <div>
